...
def create_base_scenario_nested(
    in_data: Path,
    base_year: int,
    start_year: int,
    end_year: int,
    ...
    *,
    ...
    growth_curve: Optional["CurveLike"] = None,
    growth_mode: Literal["multiply", "add"] = "multiply",
    base_scenario: str = "default",
) -> dict:
    """
    Create the base scenario using the new base structure for 20-100x performance.

    This creates a single file for the base scenario in the base folder structure:
    base/scenario_name/data.parquet

    Base scenarios are static/imported scenarios separate from parametric scenarios.
    """
    # 0) Basic introspection
    orig_cols = list_columns(in_data)
    ...
    # 4) Add scenario metadata for base scenario format
    # Add scenario_id for compatibility and mark as base scenario
    extended["scenario_id"] = base_scenario
    extended["is_base"] = True

    # For base scenarios, we don't add parameter columns since they're not parametric
    # They are static/imported scenarios that serve as foundation for parametric scenarios

    # 5) Write to base structure: base/scenario_name/data.parquet
    output_path = out_data or in_data.parent
    base_path = output_path / "base" / base_scenario

    # Ensure directory exists
    base_path.mkdir(parents=True, exist_ok=True)
    output_file = base_path / "data.parquet"

    # Write single parquet file with ZSTD compression
    extended.to_parquet(
        output_file,
        engine="pyarrow",
        compression="zstd",
    ...
    print(f"  ✅ Base scenario written to: {output_file}")
