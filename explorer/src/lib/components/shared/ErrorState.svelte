...
<div
	class="flex flex-col items-center justify-center h-full w-full p-6 bg-red-50 rounded-lg border border-red-200 {className}"
	role="alert"
	aria-live="assertive"
>
	...
	{#if onRetry}
		<button
			...
			onclick={handleRetry}
			disabled={retrying}
			class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
		>
